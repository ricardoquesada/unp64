...
0196  20 DA 01  JSR $01DA ; some have this jsr nopped, reloc doesn't happen
0199  A9 37     LDA #$37
019b  85 01     STA $01
019d  58        CLI
...
01ec  C9 DF     CMP #$DF   ;<< not fixed, found as lower as $44 for example
01ee  D0 EA     BNE $01DA
01f0  60        RTS
Because of this, $d000-dfff will be a copy of $e000-efff.
So if $2d points to >= $d000, SOMETIMES it's better save upto $ffff or:
mem[$2d]|(mem[$2e]+$10)<<8
Still it's not a rule and I don't know exactly when.
17/06/09: Implemented but still experimental, so better check extensively.
use -v to know when it does the adjustments.
28/10/09: whoops, was clearing ONLY $d000-dfff =)
*/
                ...
                for(q=0xcd;q<0xd0;q++)
                {
                    if((*(unsigned int*)(mem+p+q)&0xffff00ff)==0xa9010020)
                    {
                        ...
                        if(Unp->DebugP)
                            printf("ECA reloc active at $%04x, from $%04x-$efff to $%04x-$ffff (use -l)\n",Unp->ECAFlg&0xffff,(Unp->ECAFlg>>16)&0xffff,0x1000+((Unp->ECAFlg>>16)&0xffff));
                        break;
                    }
                }
