...
void Scn_Intros(unpstr *Unp)
{
    ...
    /* GP Intro Hack, also found applied to Dali */
    if( Unp->DepAdr==0 )
    {
        if((*(unsigned int*)(mem+0x810)==0xFBA908a0) &&
           (*(unsigned int*)(mem+0x814)==0x48AB1E20) &&
           (*(unsigned int*)(mem+0x821)==0x490792B9) &&
           (*(unsigned int*)(mem+0x82e)==0x04ED6CF2) )
        {
            Unp->Forced=0x334;
            if (Unp->DebugP)
            ...
            Unp->EndAdr=0xae;
            Unp->DepAdr=0x04b1;
            Unp->RetAdr=0x080d; // to be determined but it's $80d for both exo and dali
            Unp->StrMem=0x0801;
            if ((mem[Unp->info->end - 0x12e]) ==
                (mem[Unp->info->end - 0x12f]))
            {
                // xorvalue
                q = mem[Unp->info->end - 0x12e];
                memcpy(decrmem, mem + Unp->info->end - 0x158, 0x158);
                // decrypted buffer should start with A5 FB A0 BE
                for (i = 0;i < 0x158;i++)
                {
                    decrmem[i] ^= q;
                }
                if (*(unsigned int*)(decrmem) == 0xbea0fba5)
                {
                    Unp->EndAdr = decrmem[0x5d]; // usually $6a
                    Unp->RetAdr = 0x07fd;
                    Unp->RtAFrc = 1;
                    q = (decrmem[0x27] | decrmem[0x28] << 8);
                    if((decrmem[0x26]==0x6c)&&(q< Unp->StrMem))
                    {
                        Unp->StrMem = q;
                        if (Unp->DebugP)
                        {
                            printf("Modded Dali version detected, ");
                            printf("indirect jmp $%04x\r\n", q);
                        }
                    }
                    else
                    {
                        ...
                        {
                            printf("Modded Dali version detected, ");
                            printf("possible start $%04x\r\n", q);
                        }
                    }
                }
            }
            PrintInfo(Unp, _I_GPINTROH4);
            Unp->IdFlag=1;return;
        }
    }
}
