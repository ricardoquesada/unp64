...
void Scn_Expert(unpstr *Unp)
{
        ...
        if(Unp->DepAdr)
        {
            ...
            if(*(unsigned int*)(mem+0x835)==0x6E8D48A9)
            {
                ...
                if(p && !Unp->IdOnly)
                {
                    ...
                    {
                        if(((mem[q]^(p&0xff))==0xac) && ((mem[q+3]^(p&0xff))==0xc0) &&
    ...
    */
    ...
    /* old hack by trigsoft? similar to 2.7 */
    if(Unp->DepAdr==0)
    {
        q=0x81b;
        if( (*(unsigned int*)(mem+q+0x000)==0x852FA978) &&
            (*(unsigned int*)(mem+q+0x004)==0x8534A900) &&
            (*(unsigned int*)(mem+q+0x214)==0x000A4C05) &&
            (*(unsigned int*)(mem+q+0x06d)==0x000A4C2F) )
        {
            Unp->Forced=q;
            Unp->info->run=q;
            Unp->DepAdr=0x28;
            Unp->RTIFrc=1;
            PrintInfo(Unp,_I_EXPERTTS1);
        }
    }
    ...
    /* Star Expert / Fast Freeze by Chatterbox (Cat&Korsh)
       visible text "CBX FREEZE"
    */
    if(Unp->DepAdr==0)
    {
        if( (mem[0x80d]==0x78) &&
            (mem[0x80e]==0xa2) &&
            (*(unsigned int*)(mem+0x0810)==0x852FA99A) &&
            (*(unsigned int*)(mem+0x0821)==0x4A4A4A4A) &&
            (*(unsigned int*)(mem+0x0879)==0x88000299) &&
            (*(unsigned int*)(mem+0x087d)==0x024CF710)
          )
        {
            Unp->Forced=0x80d;
            Unp->info->run=0x80d;
