...
void Scn_PuCrunch(unpstr *Unp)
{
    ...
    if( Unp->DepAdr==0)
    {
           ...
           (*(unsigned int*)(mem+0x839)==0x01164CED) )
        {
           ...
           ((*(unsigned int*)(mem+0x811)&0xff00ffff)==0x8500A901) &&
           (*(unsigned int*)(mem+0x81d)==0xF69D083C) &&
        ...
        90 ?? C8 20 ?? 0? 85 ?? C9 ?0 90 0B A2 0? 20 ?? 0? 85 ?? 20 ?? 0? A8 20 ?? 0? AA BD ?? 0? E0 20 90 0? 8A
        (A2 03) not always
        20 ?? 02 A6
        ?? E8 20 F9
        */
        if(Unp->DepAdr==0)
        {
            ...
            for(q=0x70;q<0xff;q++)
            {
                if(((*(unsigned int*)(mem+0x801+q   )&0xFFFF00FF)==0x20C80090) &&
                   ((*(unsigned int*)(mem+0x801+q+ 8)&0xFFFF0FFF)==0x0B9000C9) &&
                   ((*(unsigned int*)(mem+0x801+q+12)&0x00FFF0FF)==0x002000A2) &&
                   ((*(unsigned int*)(mem+0x801+q+30)&0xF0FFFFFf)==0x009020E0) )
                {
