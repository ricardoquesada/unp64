...
{   /* iAN: ldx #1 lda $ff,x should fetch from $00 and not $100 */
...
{   /* iAN: ldy #1 ldx $ff,y should fetch from $00 and not $100 */
...
static void op_sbx(struct cpu_ctx *r, int mode, union inst_arg *arg)
{
    ...
    r->x &= r->a;
    r->flags = (r->flags&0xfe) | (r->x >= arg->imm.value?1:0); /* fixed: Carry IS set by SBX but not used during subtraction */
    r->x -= arg->imm.value;
    update_flags_nz(r, r->x);
}
...
/* iAN */
static void op_asr(struct cpu_ctx *r, int mode, union inst_arg *arg)
...
/* iAN */
static void op_shx(struct cpu_ctx *r, int mode, union inst_arg *arg)
{   // note: only works if address is $xx00, used in Bitpickler/Cjam
    r->mem[arg->ea.value] = r->x & (1+(arg->ea.value>>8));
}
...
static struct op_info op_shx_o = {&op_shx, "shx"};
...
static struct inst_info ops[256] = {
    ...
    {&op_shx_o, &mode_absy_o, 5},   /* $9e shx $ffff,y */
    NULL_OP,
    /* 0xa0 */
...
void next_inst(struct cpu_ctx *r)
{
    ...
    LOG(LOG_DUMP, ("%02x %02x %02x %02x %02x: ", r->a, r->x, r->y, r->sp,r->flags));
            ...
            else
            {
                ...
                case 0xcc: /* cpy $ffff */
                    if( (arg->ea.value==0xd011) || (arg->ea.value==0xd012) )
                    {
                        ...
                        bt=byted011[arg->ea.value-0xd011];
                        br=r->a;
                        if(op_code==0xec) br=r->x;
                        if(op_code==0xcc) br=r->y;
                        subtract(r, 1, br, bt);
                        WriteToIO=4;
                        break;
                    }
